...
function wrapWithDarkMode(selector, config) {
        ...
        switch (prefix) {
            case "text":
                ...
                if (suffix.includes("-")) {
                    ...
                    if (typeof colorSection === "object" && colorSection !== null && colorShade && colorSection[colorShade]) {
                        declarations.push(new postcss_1.Declaration({ prop: "color", value: colorSection[colorShade] }));
                    }
                    else if (typeof colorSection === "string") {
                        // e.g., text-primary
                        declarations.push(new postcss_1.Declaration({ prop: "color", value: colorSection }));
                    }
                }
                else {
            ...
            case "bg":
                ...
                if (suffix.includes("-")) {
                    ...
                    const colorSection = get(config.theme.colors, colorKey);
                    if (typeof colorSection === "object" && colorSection !== null && colorShade && colorSection[colorShade]) {
                        declarations.push(new postcss_1.Declaration({ prop: "background-color", value: colorSection[colorShade] }));
                    }
                    else if (typeof colorSection === "string") {
                        // e.g., bg-primary
                        declarations.push(new postcss_1.Declaration({ prop: "background-color", value: colorSection }));
                    }
                }
                else {
...
const patchedPlugin = (opts) => {
    ...
    return {
        ...
        Once(root) {
            ...
            atApplyQueue.forEach(({ rule: parentRule, classes: appliedClasses }) => {
                appliedClasses.forEach((cls) => {
                    ...
                    if (!declarationsToApply || declarationsToApply.length === 0) {
                        declarationsToApply = generateUtilityCssWithArbitrary(baseClassToApply, mergedConfig);
                    }
                    if (declarationsToApply && declarationsToApply.length > 0) {
            ...
            function wrapDarkMode(rule, darkMode) {
                if (darkMode === "class") {
                    ...
                    const media = postcss_2.default.atRule({ name: "media", params: "(prefers-color-scheme: dark)" });
            ...
            allClasses.forEach((className) => {
                ...
                if (decls.length > 0) {
                    ...
                    const rule = postcss_2.default.rule({ selector });
