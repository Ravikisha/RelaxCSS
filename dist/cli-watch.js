...
const srcGlob = `src/**/*.${fileExtensions.length > 1
    ? `{${fileExtensions.join(",")}}`
    : fileExtensions[0]}`;
...
function processFiles() {
    ...
    (0, postcss_1.default)([(0, index_1.default)()])
        ...
        fs_1.default.writeFileSync(outFile, result.css, "utf8");
        console.log(`[RelaxCSS] Rebuilt: ${outFile} (${new Date().toLocaleTimeString()})`);
    })
        .catch((err) => {
...
processFiles();
// Watch for changes
const watcher = chokidar_1.default.watch(srcGlob, { ignoreInitial: true });
