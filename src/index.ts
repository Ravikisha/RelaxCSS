import postcss, { Plugin, Root, Rule, Declaration } from "postcss";

// --- Updated RelaxConfig Interface (Expanded for more theme properties) ---
interface RelaxConfig {
  theme: {
    ...
    // Add other theme categories as needed (e.g., opacity, transition, etc.)
  };
  variants: {
    responsive: string[];
    pseudoClass: string[];
    // Add other variant types as needed (e.g., dark mode, group-hover)
  };
}
...
function relaxcss(opts: { config?: string } = {}): Plugin {
  let userConfig: RelaxConfig;
  if (opts.config) {
    try {
      ...
      userConfig = {
        ...
        theme: {
          ...
          screens: {
            ...
            ...(loadedConfig.theme?.screens || {}),
          },
          colors: {
            ...defaultConfig.theme.colors,
            ...(loadedConfig.theme?.colors || {}),
          },
          spacing: {
            ...defaultConfig.theme.spacing,
            ...(loadedConfig.theme?.spacing || {}),
          },
          borderRadius: {
            ...defaultConfig.theme.borderRadius,
            ...(loadedConfig.theme?.borderRadius || {}),
          },
          fontSizes: {
            ...defaultConfig.theme.fontSizes,
            ...(loadedConfig.theme?.fontSizes || {}),
          },
          fontWeights: {
            ...defaultConfig.theme.fontWeights,
            ...(loadedConfig.theme?.fontWeights || {}),
          },
          lineHeights: {
            ...defaultConfig.theme.lineHeights,
            ...(loadedConfig.theme?.lineHeights || {}),
          },
          boxShadow: {
            ...defaultConfig.theme.boxShadow,
            ...(loadedConfig.theme?.boxShadow || {}),
          },
          zIndex: {
            ...defaultConfig.theme.zIndex,
            ...(loadedConfig.theme?.zIndex || {}),
          },
          maxWidth: {
            ...defaultConfig.theme.maxWidth,
            ...(loadedConfig.theme?.maxWidth || {}),
          },
          minWidth: {
            ...defaultConfig.theme.minWidth,
            ...(loadedConfig.theme?.minWidth || {}),
          },
          maxHeight: {
            ...defaultConfig.theme.maxHeight,
            ...(loadedConfig.theme?.maxHeight || {}),
          },
          minHeight: {
            ...defaultConfig.theme.minHeight,
            ...(loadedConfig.theme?.minHeight || {}),
          },
        },
        variants: {
      ...
      console.error(
        `RelaxCSS: Could not load config file at ${opts.config}. Using default configuration.`,
        e
      );
  ...
  const getVariant = (cls: string, variantTypes: string[]): string | null => {
    for (const v of variantTypes) {
      if (cls.startsWith(`${v}:`)) return v;
    }
    return null;
  };
  ...
  return {
    ...
    Once(root: Root) {
      ...
      const classesToGenerateUtilitiesFor = new Set<string>();
      const atApplyQueue: { parentRule: Rule; appliedClass: string }[] = [];
      ...
      atApplyQueue.forEach(({ parentRule, appliedClass }) => {
        ...
        let responsiveVariant: string | null = null;
        let pseudoClassVariant: string | null = null;
        let baseUtility: string = "";

        // Extract responsive variant
        if (
          userConfig.variants.responsive &&
          Array.isArray(userConfig.variants.responsive)
        ) {
          responsiveVariant = getVariant(
            currentClass,
            userConfig.variants.responsive
          );
        ...
        if (
          userConfig.variants.pseudoClass &&
          Array.isArray(userConfig.variants.pseudoClass)
        ) {
          pseudoClassVariant = getVariant(
            currentClass,
            userConfig.variants.pseudoClass
          );
        ...
        if (!baseRuleDeclarations) return; // If no declarations are generated for the base utility, skip

        let targetRule: Rule = parentRule;
        let mediaQueryRule: postcss.AtRule | null = null;

        if (responsiveVariant) {
          if (!userConfig.theme.screens[responsiveVariant]) {
            console.warn(
              `RelaxCSS: Responsive variant '${responsiveVariant}' used in @apply for '${appliedClass}' but no screen size defined in config. Skipping.`
            );
            return;
          }
          ...
          mediaQueryRule = root.nodes.find(
            (node) =>
              node.type === "atrule" &&
              node.name === "media" &&
          ...
          if (!mediaQueryRule.nodes) {
            (mediaQueryRule as any).nodes = []; // Cast to any to add nodes if missing
          }
          ...
          let existingRuleInMq = (mediaQueryRule.nodes || []).find(
            (node) => node.type === "rule" && node.selector === innerSelector
          ) as Rule;

          if (!existingRuleInMq) {
            existingRuleInMq = postcss.rule({ selector: innerSelector });
          ...
          let existingVariantRule = parentRule.parent?.nodes.find(
            (node) => node.type === "rule" && node.selector === variantSelector
          ) as Rule;

          if (!existingVariantRule) {
            existingVariantRule = postcss.rule({ selector: variantSelector });
      ...
      const utilities: (Rule | postcss.AtRule)[] = [];
      ...
      classesToGenerateUtilitiesFor.forEach((cls) => {
        ...
        const baseRuleDeclarations = generateRuleFor(baseUtility, userConfig);
        if (!baseRuleDeclarations) return;

        // Construct the final selector for the utility
        let newRuleSelector: string = `.${cls
          .replace(/([:[]])/g, "\\$1")
        ...
        const finalUtilityRule = postcss.rule({ selector: newRuleSelector });
        baseRuleDeclarations.nodes.forEach((node) =>
          finalUtilityRule.append(node.clone())
        );

        if (responsiveVariant) {
          if (!userConfig.theme.screens[responsiveVariant]) {
            console.warn(
              `RelaxCSS: Responsive variant '${responsiveVariant}' used in utility '${cls}' but no screen size defined in config. Skipping.`
            );
            return;
          }
      ...
      utilities.sort((a, b) => {
        ...
        if (isAMedia && !isBMedia) return 1; // Media queries come after regular rules
        if (!isAMedia && isBMedia) return -1; // Regular rules come before media queries
        if (isAMedia && isBMedia) {
          // Sort media queries by breakpoint value
          const getBreakpoint = (node: postcss.AtRule) => {
            const match = node.params.match(/min-width:\s*(\d+)/);
            return match ? parseInt(match[1]) : 0;
          };
          return (
            getBreakpoint(a as postcss.AtRule) -
            getBreakpoint(b as postcss.AtRule)
          );
        }
...
function generateRuleFor(cls: string, cfg: RelaxConfig): Rule | null {
  const rule = postcss.rule({ selector: `.${cls}` }); // This selector is a placeholder/temp

  // Helper to extract arbitrary value
  const getArbitraryValue = (
    className: string,
    prefix: string
  ): string | null => {
  ...
  const getValue = (
    className: string,
    prefix: string,
    themeLookup: Record<string, string>,
    prop: string // for logging/debugging
  ): string | null => {
  ...
  // --- Modular Utility Generators ---
  interface UtilityHandler {
    prop: string | string[]; // Can be a single property or an array for multiple
    themeKey?: keyof RelaxConfig["theme"]; // Key in the theme object (e.g., 'colors', 'spacing')
    valueMap?: Record<string, string | string[]>; // Direct map for specific keywords (e.g., 'block' -> 'block', 'truncate' -> ['hidden', 'ellipsis', 'nowrap'])
    complexHandler?: (key: string, cfg: RelaxConfig) => Declaration[]; // For complex cases like flex
  }

  const utilityMap: Record<string, UtilityHandler> = {
    // Colors
    ...
    "text-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        if (color) {
          declarations.push(postcss.decl({ prop: "color", value: color }));
        }
        // Fallback to font-size if not a color
        const fontSize = cfg.theme.fontSizes[key];
        if (fontSize) {
          declarations.push(
            postcss.decl({ prop: "font-size", value: fontSize })
          );
        }
        return declarations;
      },
    ...
    "border-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        if (color) {
          declarations.push(
            postcss.decl({ prop: "border-color", value: color })
          );
        }
        // Then try to get border-width (e.g., border-2, border-0)
        const value =
          getArbitraryValue(`border-[${key}]`, "border-") ||
          cfg.theme.spacing[key];
        if (value !== undefined) {
          declarations.push(
            postcss.decl({ prop: "border-width", value: value })
          );
        } else if (key === "") {
          // Handles plain 'border' class, default to 1px
          declarations.push(
            postcss.decl({ prop: "border-width", value: "1px" })
          );
        }
        return declarations;
      },
    ...
    "-m-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        return value
          ? [postcss.decl({ prop: "margin", value: `-${value}` })]
          : [];
      },
    },
    "-mt-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        return value
          ? [postcss.decl({ prop: "margin-top", value: `-${value}` })]
          : [];
      },
    },
    "-mb-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        return value
          ? [postcss.decl({ prop: "margin-bottom", value: `-${value}` })]
          : [];
      },
    },
    "-ml-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        return value
          ? [postcss.decl({ prop: "margin-left", value: `-${value}` })]
          : [];
      },
    },
    "-mr-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        return value
          ? [postcss.decl({ prop: "margin-right", value: `-${value}` })]
          : [];
      },
    },
    "-mx-": {
    ...
    "inset-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        return value
          ? [
              postcss.decl({ prop: "top", value }),
              postcss.decl({ prop: "right", value }),
              postcss.decl({ prop: "bottom", value }),
              postcss.decl({ prop: "left", value }),
            ]
          : [];
      },
    },
    ...
    "font-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        if (key === "italic")
          declarations.push(
            postcss.decl({ prop: "font-style", value: "italic" })
          );
        if (key === "not-italic")
          declarations.push(
            postcss.decl({ prop: "font-style", value: "normal" })
          );
        return declarations;
      },
    },
    ...
    "grid-cols-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        if (key === "none")
          return [
            postcss.decl({ prop: "grid-template-columns", value: "none" }),
          ];
    ...
    "col-span-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        if (key === "full")
          return [postcss.decl({ prop: "grid-column", value: "1 / -1" })];
        return [];
      },
    },
    "col-start-": {
      ...
      complexHandler: (key, cfg) => {
        // Added cfg argument
        if (key === "auto")
          return [postcss.decl({ prop: "grid-column-start", value: "auto" })];
        const num = parseInt(key);
        if (!isNaN(num) && num > 0) {
          return [postcss.decl({ prop: "grid-column-start", value: key })];
        }
        return [];
      },
    },
    "col-end-": {
      ...
      complexHandler: (key, cfg) => {
        // Added cfg argument
        if (key === "auto")
          return [postcss.decl({ prop: "grid-column-end", value: "auto" })];
        const num = parseInt(key);
        if (!isNaN(num) && num > 0) {
          return [postcss.decl({ prop: "grid-column-end", value: key })];
        }
        return [];
      },
    },
    "grid-rows-": {
      ...
      complexHandler: (key, cfg) => {
        // Added cfg argument
        if (key === "none")
          return [postcss.decl({ prop: "grid-template-rows", value: "none" })];
        const num = parseInt(key);
        if (!isNaN(num) && num > 0) {
          return [
            postcss.decl({
    ...
    "row-span-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        if (key === "full")
          return [postcss.decl({ prop: "grid-row", value: "1 / -1" })];
        return [];
      },
    },
    "row-start-": {
      ...
      complexHandler: (key, cfg) => {
        // Added cfg argument
        if (key === "auto")
          return [postcss.decl({ prop: "grid-row-start", value: "auto" })];
        const num = parseInt(key);
        if (!isNaN(num) && num > 0) {
          return [postcss.decl({ prop: "grid-row-start", value: key })];
        }
        return [];
      },
    },
    "row-end-": {
      ...
      complexHandler: (key, cfg) => {
        // Added cfg argument
        if (key === "auto")
          return [postcss.decl({ prop: "grid-row-end", value: "auto" })];
        const num = parseInt(key);
        if (!isNaN(num) && num > 0) {
          return [postcss.decl({ prop: "grid-row-end", value: key })];
        }
        return [];
      },
    },
    ...
    "border-t-": {
      ...
      complexHandler: (key, cfg) => {
        const value =
          getArbitraryValue(`border-t-[${key}]`, "border-t-") ||
          cfg.theme.spacing[key] ||
          "1px"; // Default to 1px if no theme value for general border
        return [postcss.decl({ prop: "border-top-width", value: value })];
      },
    },
    "border-b-": {
      ...
      complexHandler: (key, cfg) => {
        const value =
          getArbitraryValue(`border-b-[${key}]`, "border-b-") ||
          cfg.theme.spacing[key] ||
          "1px";
        return [postcss.decl({ prop: "border-bottom-width", value: value })];
      },
    },
    "border-l-": {
      ...
      complexHandler: (key, cfg) => {
        const value =
          getArbitraryValue(`border-l-[${key}]`, "border-l-") ||
          cfg.theme.spacing[key] ||
          "1px";
        return [postcss.decl({ prop: "border-left-width", value: value })];
      },
    },
    "border-r-": {
      ...
      complexHandler: (key, cfg) => {
        const value =
          getArbitraryValue(`border-r-[${key}]`, "border-r-") ||
          cfg.theme.spacing[key] ||
          "1px";
        return [postcss.decl({ prop: "border-right-width", value: value })];
      },
    },
    ...
    shadow: {
      ...
      complexHandler: (key, cfg) => {
        ...
        const shadowValue =
          cfg.theme.boxShadow[key] || cfg.theme.boxShadow.DEFAULT;
        return shadowValue
          ? [postcss.decl({ prop: "box-shadow", value: shadowValue })]
          : [];
      },
    },
    ...
    "opacity-": {
      ...
      complexHandler: (key, cfg) => {
        ...
        if (!isNaN(num) && num >= 0 && num <= 100) {
          // e.g., opacity-50
          declarations.push(
            postcss.decl({ prop: "opacity", value: (num / 100).toString() })
          );
        }
        ...
        if (arbitrary) {
          declarations.push(
            postcss.decl({ prop: "opacity", value: arbitrary })
          );
        }
        return declarations;
      },
    ...
    if (cls.startsWith(prefix)) {
      ...
      if (arbitraryValue !== null) {
        const props = Array.isArray(handler.prop)
          ...
          : [handler.prop];
        props.forEach((p) =>
          declarations.push(postcss.decl({ prop: p, value: arbitraryValue }))
        );
        ...
        if (Array.isArray(value)) {
          ...
          if (props.length === value.length) {
            value.forEach((val, index) => {
              declarations.push(
                postcss.decl({ prop: props[index], value: val })
              );
            });
          } else {
            console.warn(
              `RelaxCSS: Mismatch in prop and value array length for '${cls}'.`
            );
          }
        } else {
          props.forEach((p) =>
            declarations.push(postcss.decl({ prop: p, value: value as string }))
          ); // Type assertion
        }
        ...
        if (themeCategory && typeof themeCategory === "object") {
          ...
          const themeValue = (themeCategory as Record<string, string>)[key];
          if (themeValue !== undefined) {
            const props = Array.isArray(handler.prop)
              ? handler.prop
              : [handler.prop];
            props.forEach((p) =>
              declarations.push(postcss.decl({ prop: p, value: themeValue }))
            );
          }
        }
      }
  ...
  switch (cls) {
    ...
    case "antialiased":
      rule.append(
        postcss.decl({ prop: "-webkit-font-smoothing", value: "antialiased" })
      );
      rule.append(
        postcss.decl({ prop: "-moz-osx-font-smoothing", value: "grayscale" })
      );
      return rule;
    case "subpixel-antialiased":
      rule.append(
        postcss.decl({ prop: "-webkit-font-smoothing", value: "auto" })
      );
      rule.append(
        postcss.decl({ prop: "-moz-osx-font-smoothing", value: "auto" })
      );
      return rule;
    case "table":
      rule.append(postcss.decl({ prop: "display", value: "table" }));
      return rule;
    case "table-row":
      rule.append(postcss.decl({ prop: "display", value: "table-row" }));
      return rule;
    case "table-cell":
      rule.append(postcss.decl({ prop: "display", value: "table-cell" }));
      return rule;
