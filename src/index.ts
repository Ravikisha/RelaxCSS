import { PluginCreator, Root, Rule, Declaration } from "postcss";
import postcss from "postcss";
import merge from "lodash.merge";

// --- 1. Define RelaxConfig Interface and Default Configuration ---
...
function generateUtilityCss(
  ...
  const applySpacingProperty = (
    propBase: "padding" | "margin" | "top" | "right" | "bottom" | "left",
    valueKey: string,
    axisOrSide?: string
  ) => {
    const value = get(config.theme.spacing, valueKey);

    if (value === undefined && valueKey !== "auto") return; // Allow 'auto' for margin

    const finalValue =
      valueKey === "auto" && propBase.startsWith("margin") ? "auto" : value;

    switch (axisOrSide) {
      case "x":
        declarations.push(
          new Declaration({ prop: `${propBase}-left`, value: finalValue }),
          new Declaration({ prop: `${propBase}-right`, value: finalValue })
        );
        break;
      case "y":
        declarations.push(
          new Declaration({ prop: `${propBase}-top`, value: finalValue }),
          new Declaration({ prop: `${propBase}-bottom`, value: finalValue })
        );
        break;
      case "t":
        declarations.push(
      ...
      default: // No axis or side, apply to all
        declarations.push(
          new Declaration({ prop: propBase, value: finalValue })
        );
        break;
    }
  };
  ...
  if (className.startsWith("p-") || className.startsWith("m-")) {
    ...
    if (value !== undefined)
      declarations.push(new Declaration({ prop: "max-height", value }));
  }
    ...
    switch (prefix) {
      case "text":
        ...
        if (suffix.includes("-")) {
          ...
          if (Array.isArray(fontSizeTuple)) {
            declarations.push(
              new Declaration({ prop: "font-size", value: fontSizeTuple[0] })
            );
            if (fontSizeTuple[1]?.lineHeight) {
              declarations.push(
                new Declaration({
                  prop: "line-height",
                  value: fontSizeTuple[1].lineHeight as string,
                })
              );
            }
          }
      ...
      case "font":
        ...
        if (fontWeightValue !== undefined)
          declarations.push(
            new Declaration({ prop: "font-weight", value: fontWeightValue })
          );
        break;
      case "leading":
        ...
        if (lineHeightValue !== undefined)
          declarations.push(
            new Declaration({ prop: "line-height", value: lineHeightValue })
          );
        break;
      case "shadow":
        ...
        if (shadowValue !== undefined)
          declarations.push(
            new Declaration({ prop: "box-shadow", value: shadowValue })
          );
        break;
      case "z":
        ...
        if (zIndexValue !== undefined)
          declarations.push(
            new Declaration({ prop: "z-index", value: zIndexValue })
          );
        break;
      case "opacity":
        ...
        if (opacityValue !== undefined)
          declarations.push(
            new Declaration({ prop: "opacity", value: opacityValue })
          );
        break;
      case "rounded":
        const borderRadiusValue = get(
      ...
      case "border":
        if (suffix.includes("-")) {
          ...
          if (colorValue)
            declarations.push(
              new Declaration({ prop: "border-color", value: colorValue })
            );
          else {
            ...
            if (widthValue !== undefined)
              declarations.push(
                new Declaration({ prop: "border-width", value: widthValue })
              );
          }
        } else {
          ...
          declarations.push(
            ...
            new Declaration({ prop: "border-style", value: "solid" }),
            new Declaration({ prop: "border-color", value: "currentColor" })
          );
        }
        break;
      case "grid":
        if (suffix === "cols") {
          ...
          if (value)
            declarations.push(
              new Declaration({ prop: "grid-template-columns", value })
            );
        } else if (suffix === "rows") {
          const value = get(config.theme.gridTemplateRows, suffix);
          if (value)
            declarations.push(
              new Declaration({ prop: "grid-template-rows", value })
            );
        } else if (suffix === "") {
          // display: grid
          declarations.push(
            new Declaration({ prop: "display", value: "grid" })
          );
        }
        break;
      case "col":
        if (suffix.startsWith("span-")) {
          ...
          if (start)
            declarations.push(
              new Declaration({ prop: "grid-column-start", value: start })
            );
        } else if (suffix.startsWith("end-")) {
          const end = get(config.theme.spacing, suffix.substring(4));
          if (end)
            declarations.push(
              new Declaration({ prop: "grid-column-end", value: end })
            );
        } else if (suffix === "auto") {
          declarations.push(
            new Declaration({ prop: "grid-column", value: "auto" })
          );
        }
        break;
      case "row":
        if (suffix.startsWith("span-")) {
          ...
          if (start)
            declarations.push(
              new Declaration({ prop: "grid-row-start", value: start })
            );
        } else if (suffix.startsWith("end-")) {
          const end = get(config.theme.spacing, suffix.substring(4));
          if (end)
            declarations.push(
              new Declaration({ prop: "grid-row-end", value: end })
            );
        } else if (suffix === "auto") {
          declarations.push(
            new Declaration({ prop: "grid-row", value: "auto" })
          );
        }
        break;
      case "gap":
        if (suffix.startsWith("x-")) {
          ...
          if (value)
            declarations.push(new Declaration({ prop: "column-gap", value }));
        } else if (suffix.startsWith("y-")) {
          const value = get(config.theme.spacing, suffix.substring(2));
          if (value)
            declarations.push(new Declaration({ prop: "row-gap", value }));
        } else {
          const value = get(config.theme.spacing, suffix);
          if (value) declarations.push(new Declaration({ prop: "gap", value }));
        }
        break;
      case "auto":
      ...
      case "static":
        declarations.push(
          new Declaration({ prop: "position", value: "static" })
        );
        break;
      case "relative":
        declarations.push(
      ...
      case "absolute":
        declarations.push(
          new Declaration({ prop: "position", value: "absolute" })
        );
        break;
      case "fixed":
        declarations.push(
          new Declaration({ prop: "position", value: "fixed" })
        );
        break;
      case "sticky":
        declarations.push(
          new Declaration({ prop: "position", value: "sticky" })
        );
        break;
      case "top":
      case "right":
      ...
      case "left":
        ...
        if (positionValue !== undefined)
          declarations.push(
            new Declaration({ prop: prefix, value: positionValue })
          );
        break;
      case "inset":
        if (suffix.startsWith("x-")) {
          ...
          if (value !== undefined)
            declarations.push(
              new Declaration({ prop: "left", value }),
              new Declaration({ prop: "right", value })
            );
        } else if (suffix.startsWith("y-")) {
          const value = get(config.theme.spacing, suffix.substring(2));
          if (value !== undefined)
            declarations.push(
              new Declaration({ prop: "top", value }),
              new Declaration({ prop: "bottom", value })
            );
        } else {
          const value = get(config.theme.spacing, suffix);
          if (value !== undefined)
      ...
      case "overflow":
        if (
          ...
          declarations.push(
            new Declaration({ prop: "overflow", value: suffix })
          );
        } else if (suffix.startsWith("x-")) {
          declarations.push(
            new Declaration({ prop: "overflow-x", value: suffix.substring(2) })
          );
        } else if (suffix.startsWith("y-")) {
          declarations.push(
            new Declaration({ prop: "overflow-y", value: suffix.substring(2) })
          );
        }
        break;
      case "mix-blend":
        declarations.push(
          new Declaration({ prop: "mix-blend-mode", value: suffix })
        );
        break;
      case "background-blend":
        declarations.push(
          new Declaration({ prop: "background-blend-mode", value: suffix })
        );
        break;
      ...
      case "backdrop-sepia":
        declarations.push(
          new Declaration({ prop: `--relax-filter`, value: `${className}` })
        );
        break;
      case "scale":
      case "rotate":
      ...
      case "skew":
        declarations.push(
          new Declaration({ prop: `--relax-transform`, value: `${className}` })
        );
        break;
      case "origin":
        declarations.push(
      ...
      case "transition":
        ...
        if (prop !== undefined)
          declarations.push(
            new Declaration({ prop: "transition-property", value: prop })
          );
        break;
      case "duration":
        ...
        if (duration !== undefined)
          declarations.push(
            new Declaration({ prop: "transition-duration", value: duration })
          );
        break;
      case "ease":
      ...
      case "delay":
        ...
        if (delayValue !== undefined)
          declarations.push(
            new Declaration({ prop: "transition-delay", value: delayValue })
          );
        break;

      case "pointer-events":
        declarations.push(
          new Declaration({ prop: "pointer-events", value: suffix })
        );
        break;
      case "cursor":
        declarations.push(new Declaration({ prop: "cursor", value: suffix }));
        break;
      case "resize":
        if (suffix === "")
          declarations.push(new Declaration({ prop: "resize", value: "both" }));
        else if (suffix === "x")
          declarations.push(
            new Declaration({ prop: "resize", value: "horizontal" })
          );
        else if (suffix === "y")
          declarations.push(
            new Declaration({ prop: "resize", value: "vertical" })
          );
        else if (suffix === "none")
          declarations.push(new Declaration({ prop: "resize", value: "none" }));
        break;
      case "select":
        declarations.push(
          new Declaration({
      ...
      case "sr":
        if (suffix === "only") {
          declarations.push(
            new Declaration({ prop: "position", value: "absolute" }),
            new Declaration({ prop: "width", value: "1px" }),
            new Declaration({ prop: "height", value: "1px" }),
            new Declaration({ prop: "padding", value: "0" }),
            new Declaration({ prop: "margin", value: "-1px" }),
            new Declaration({ prop: "overflow", value: "hidden" }),
            new Declaration({ prop: "clip", value: "rect(0, 0, 0, 0)" }),
            new Declaration({ prop: "white-space", value: "nowrap" }),
      ...
      case "not":
        if (suffix === "sr-only") {
          declarations.push(
            new Declaration({ prop: "position", value: "static" }),
            new Declaration({ prop: "width", value: "auto" }),
            new Declaration({ prop: "height", value: "auto" }),
            new Declaration({ prop: "padding", value: "0" }),
            new Declaration({ prop: "margin", value: "0" }),
            new Declaration({ prop: "overflow", value: "visible" }),
            new Declaration({ prop: "clip", value: "auto" }),
            new Declaration({ prop: "white-space", value: "normal" })
          );
        }
        break;
      case "w": // width
        ...
        if (widthValue !== undefined)
          declarations.push(
            new Declaration({ prop: "width", value: widthValue })
          );
        break;
      case "h": // height
        ...
        if (heightValue !== undefined)
          declarations.push(
            new Declaration({ prop: "height", value: heightValue })
          );
        break;
      case "min-w":
        ...
        if (minWidthValue !== undefined)
          declarations.push(
            new Declaration({ prop: "min-width", value: minWidthValue })
          );
        break;
      case "min-h":
        ...
        if (minHeightValue !== undefined)
          declarations.push(
            new Declaration({ prop: "min-height", value: minHeightValue })
          );
        break;
      case "flex-grow":
        if (suffix === "" || suffix === "0")
          declarations.push(new Declaration({ prop: "flex-grow", value: "0" }));
        else
          declarations.push(new Declaration({ prop: "flex-grow", value: "1" }));
        break;
      case "flex-shrink":
        if (suffix === "" || suffix === "0")
          declarations.push(
            new Declaration({ prop: "flex-shrink", value: "0" })
          );
        else
          declarations.push(
            new Declaration({ prop: "flex-shrink", value: "1" })
          );
        break;
      case "justify-items":
        declarations.push(
          new Declaration({ prop: "justify-items", value: suffix })
        );
        break;
      case "justify-self":
        declarations.push(
          new Declaration({ prop: "justify-self", value: suffix })
        );
        break;
      case "place-items":
        declarations.push(
          new Declaration({ prop: "place-items", value: suffix })
        );
        break;
      case "place-content":
        declarations.push(
          new Declaration({ prop: "place-content", value: suffix })
        );
        break;
      case "place-self":
        declarations.push(
          new Declaration({ prop: "place-self", value: suffix })
        );
        break;
      case "container": // Example of a pre-defined component/utility
        ...
        declarations.push(
          new Declaration({ prop: "width", value: "100%" }),
          new Declaration({ prop: "margin-right", value: "auto" }),
          new Declaration({ prop: "margin-left", value: "auto" })
          // Responsive max-width would typically be handled by the user adding sm:max-w-xl etc.
        );
        break;
      case "block":
        declarations.push(new Declaration({ prop: "display", value: "block" }));
        break;
      case "inline":
        declarations.push(
          new Declaration({ prop: "display", value: "inline" })
        );
        break;
      case "inline-block":
        declarations.push(
          new Declaration({ prop: "display", value: "inline-block" })
        );
        break;
      case "hidden":
        declarations.push(new Declaration({ prop: "display", value: "none" }));
        break;
      case "flex":
        if (suffix === "")
          declarations.push(
          ...
          declarations.push(
            new Declaration({ prop: "flex-direction", value: "row" })
          );
        else if (suffix === "col")
          declarations.push(
            new Declaration({ prop: "flex-direction", value: "column" })
          );
        else if (suffix === "row-reverse")
          declarations.push(
            new Declaration({ prop: "flex-direction", value: "row-reverse" })
          );
        else if (suffix === "col-reverse")
          declarations.push(
            new Declaration({ prop: "flex-direction", value: "column-reverse" })
          );
        else if (suffix === "wrap")
          declarations.push(
            new Declaration({ prop: "flex-wrap", value: "wrap" })
          );
        else if (suffix === "wrap-reverse")
          declarations.push(
            new Declaration({ prop: "flex-wrap", value: "wrap-reverse" })
          );
        else if (suffix === "nowrap")
          declarations.push(
            new Declaration({ prop: "flex-wrap", value: "nowrap" })
          );
        else if (suffix === "1")
          declarations.push(new Declaration({ prop: "flex", value: "1 1 0%" }));
        else if (suffix === "auto")
          declarations.push(
            new Declaration({ prop: "flex", value: "1 1 auto" })
          );
        else if (suffix === "initial")
          declarations.push(
            new Declaration({ prop: "flex", value: "0 1 auto" })
          );
        else if (suffix === "none")
          declarations.push(new Declaration({ prop: "flex", value: "none" }));
        break;
      case "items":
        if (suffix === "start")
          declarations.push(
            new Declaration({ prop: "align-items", value: "flex-start" })
          );
        else if (suffix === "end")
          declarations.push(
            new Declaration({ prop: "align-items", value: "flex-end" })
          );
        else if (suffix === "center")
          declarations.push(
            new Declaration({ prop: "align-items", value: "center" })
          );
        else if (suffix === "baseline")
          declarations.push(
            new Declaration({ prop: "align-items", value: "baseline" })
          );
        else if (suffix === "stretch")
          declarations.push(
            new Declaration({ prop: "align-items", value: "stretch" })
          );
        break;
      case "justify":
        if (suffix === "start")
          declarations.push(
            new Declaration({ prop: "justify-content", value: "flex-start" })
          );
        else if (suffix === "end")
          declarations.push(
            new Declaration({ prop: "justify-content", value: "flex-end" })
          );
        else if (suffix === "center")
          declarations.push(
            new Declaration({ prop: "justify-content", value: "center" })
          );
        else if (suffix === "between")
          declarations.push(
            new Declaration({ prop: "justify-content", value: "space-between" })
          );
        else if (suffix === "around")
          declarations.push(
            new Declaration({ prop: "justify-content", value: "space-around" })
          );
        else if (suffix === "evenly")
          declarations.push(
            new Declaration({ prop: "justify-content", value: "space-evenly" })
          );
        break;
      case "align-self":
        declarations.push(
          new Declaration({ prop: "align-self", value: suffix })
        );
        break;
      case "box-border":
        declarations.push(
      ...
      case "box-content":
        declarations.push(
          new Declaration({ prop: "box-sizing", value: "content-box" })
        );
        break;
    }
  }
  ...
  if (declarations.length === 0) {
    if (config.theme.colors[className]) {
      ...
      if (typeof color === "string") {
        declarations.push(new Declaration({ prop: "color", value: color }));
      }
    }
  }
...
const patchedPlugin: PluginCreator<Partial<RelaxConfig>> = (opts) => {
  ...
  // --- Plugin System: User-defined Utilities ---
  let userUtilities: Record<string, (config: RelaxConfig) => Declaration[]> =
    {};
  if (Array.isArray(mergedConfig.plugins)) {
    mergedConfig.plugins.forEach((pluginFn) => {
  ...
  return {
    ...
    Once(root: Root) {
      ...
      const generatedUtilityDeclarations: Map<string, Declaration[]> =
        new Map();
      ...
      // add the user-defined utilities
      Object.keys(userUtilities).forEach((utilityName) => {
        allPossibleUtilityClasses.add(utilityName);
      });

      // Generate declarations for all base utilities and store them
      allPossibleUtilityClasses.forEach((className) => {
        let declarations;
        if (userUtilities[className]) {
          declarations = userUtilities[className](mergedConfig);
        } else {
          declarations = generateUtilityCss(className, mergedConfig);
        }
        if (declarations.length > 0) {
      ...
      atApplyQueue.forEach(({ rule: parentRule, classes: appliedClasses }) => {
        appliedClasses.forEach((cls) => {
          ...
          if (!declarationsToApply || declarationsToApply.length === 0) {
            declarationsToApply = generateUtilityCssWithArbitrary(
              baseClassToApply,
              mergedConfig
            );
          }
